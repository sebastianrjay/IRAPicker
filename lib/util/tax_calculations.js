...
export const accountBalanceAtRetirement = ({
  ...
  console.log('typeof totalYears', typeof totalYears)
  for (let elapsedYears = 1; elapsedYears <= totalYears; elapsedYears++) {
  // for (let elapsedYears = 1; elapsedYears <= 1000; elapsedYears++) {
...
export const beforeCapitalGainsTaxRetirementIncome = (props) => {
  ...
  return beforeTaxIncome(merge({}, props, capitalGainsTaxProps))
}

export const beforeIncomeTaxIncome = (props) => {
  const incomeTaxProps = {
    afterTaxIncomeFn: afterIncomeTaxIncome,
    upperBoundTaxMultiplier: UPPER_BOUND_INCOME_TAX_MULTIPLIER,
  }

  return beforeTaxIncome(merge(incomeTaxProps, props))
}
...
export const beforeTaxIncome = ({
  ...
  if (toDollarAmount(incomeUpperBound - .01) <= incomeLowerBound &&
    ...
    return beforeIncomeTaxIncome(merge(props, {
      annualIncome,
      incomeLowerBound,
...
export const traditionalIRATaxDeduction = ({
  ...
  if ((!incomeLimit && incomeLimit !== 0) || annualIncome < incomeLimit) {
...
export const traditionalIRAWithdrawal = (props) => {
  ...
  return beforeIncomeTaxIncome(beforeTaxIncomeProps)
}
